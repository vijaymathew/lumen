...
| Look intensity slider | ✅ | `out = mix(input, lut(input), t)` in both export and shader (intensity rides in `PreviewState`); `LooksPanel` (Load…/Clear + intensity slider); blend unit-tested + preview==export verified. **LUT bytes are embedded in the `.lumen`** (base64 in `LutNode` state via `Lut3D::fromCubeData`/`fromHaldData`) so projects are self-contained/portable — restored without the original file (legacy path-only projects still load by path). `project_test` round-trips WB + grade + an embedded LUT after deleting the source `.cube` |
...
| White balance | ✅ | **v2: true linear-light Planckian WB.** New `core/WhiteBalance` turns an absolute **Kelvin** temperature (Planckian/blackbody locus) + green/magenta **tint** into a **3×3 matrix applied in linear light** (`encoded → ^2.2 → W·rgb → ^(1/2.2)`), shared by `TuneNode::apply()` (per-pixel float pass) and the GPU shader (`PreviewState` carries the 3×3 as 9 floats `wb00..wb22`; vert+frag rebuild a `mat3`; UBO 176→208) so **preview==export**. With a RAW camera profile (`RawLoader` captures LibRaw `rgb_cam`/`cam_xyz`/`cam_mul`; `TuneNode::setCameraProfile`) the matrix is `C·diag(m₁(K,t)/m₁(K_asshot,0))·C⁻¹` — the **camera-accurate** equivalent of redoing WB in sensor space; the slider seeds at the **as-shot temperature** (identity at default). Non-RAW degrades to the diagonal sRGB model (neutral at 6500 K). `TonePanel` Temperature row is now a Kelvin slider, plus an **"As shot"** reset (back to the camera's as-shot temperature) and a **"Pick neutral" eyedropper** (`wb::solveNeutral` does a coarse-to-fine 2D K/tint search so a clicked grey patch reads neutral; reuses the canvas colour-pick mode routed by `PickPurpose`). Profile persisted in `.lumen` and re-supplied from the decode on open/load. `whitebalance_test` (locus, identity-at-reference, warming direction, tint, Kelvin estimation, mat3 inverse) + `tune_node_test` (Kelvin API, apply, profile seeding). **Limitation:** strong WB shifts can band/clip in the *8-bit preview* until the float-preview stage; the float **export** is correct. The earlier encoded-space `wbGains` hack is removed |
| Color grading (`ColorGradeNode`) | ✅ | Creative grading via **Lift / Gamma / Gain colour wheels** (ASC-CDL **Slope/Offset/Power**): per channel `out = (v·slope + offset)^power` in **encoded** display-referred space (conventional for LGG — deliberately *not* linear like WB). Each wheel = a luma-neutral 2D chroma push + a master luma term, resolved by one shared `ColorGradeNode::resolveSOP` used by `apply()` (per-pixel float pass) and the GPU shader; `PreviewState` carries 10 floats (`gradeEnabled` + slope/offset/power), shader applies after the tone curves, before the LUT (tune→curves→**grade**→lut→mono); UBO 208→240. New reusable **`ColorWheel`** widget (hue/sat disc + draggable puck, double-click recenters) ×3 in a floating **`ColorGradePanel`** (enable toggle + master sliders), opened via the **"Color grading"** command. Per-layer + maskable like other pointwise nodes; persisted in `.lumen`; undo via `NodeFactory`. `colorgrade_test` (neutral identity, gain/lift/gamma masters, red-ward chroma push, preview-state mapping, save/restore). Split-toning falls out as the shadows/highlights case. **Out of scope:** HSL hue-band mixer (separate future `ColorMixNode`). Same 8-bit-preview banding caveat as WB/LUT; export is full-float |
